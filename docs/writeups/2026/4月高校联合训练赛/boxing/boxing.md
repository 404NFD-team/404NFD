...
smtp.emial.cloud 
1. 解析其 IPv4 地址：39.109.116.223 
2. 末段 223 完全符合题目提示 
3. 按格式 flag{xxxxxx}完善得到 flag{39.109.116. 
...
6.发现完全符合，找到经纬度，得到 flag{35.682_139.758} 
...
6c 65 / 7d 00 -> 6c657d00 8、将上述十六进制序列转换为字符串： 
66 6c 61 67 -> flag 
7b 4d 31 70 -> {M1p 
35 5f 48 65 -> 5_He 
78 5f 43 30 -> x_C0 
...
即： 
对输入进行加密，若结果等于 data 则通过校验 4、发现关键数据 
...
def tea_decrypt(v, k): 
 v0, v1 = v  delta = 0x9e3779b9 
 s = (delta * 32) & 0xffffffff 
 
 for _ in range(32): 
 v1 = (v1 - (((v0 << 4) + k[2]) ^ (v0 + s) ^ ((v0 >> 
5) + k[3]))) & 0xffffffff 
 v0 = (v0 - (((v1 << 4) + k[0]) ^ (v1 + s) ^ ((v1 >> 
5) + k[1]))) & 0xffffffff 
...
cipher = bytes([ 
0x59,0x0C,0xEE,0x3D,0x4C,0x5B,0xE2,0xE8, 
0x97,0xF7,0xEF,0x5C,0xFA,0xD3,0x6F,0xB9, 0x03,0xBF,0x31,0x5A,0x80,0xD2,0xF1,0x61, 
0xD9,0x2F,0xC3,0x79,0x9A,0x91,0xDE,0x30 
]) 
 
res = b'' 
for i in range(0, len(cipher), 8): 
 v0, v1 = struct.unpack("<2I", cipher[i:i+8]) # 小端 
